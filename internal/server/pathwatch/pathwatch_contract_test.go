...
// Statement-count contract on the ingest hot path, mirroring outage's
// TestApplyIssuesOneBulkStateUpsert: however many series and changes a
// push carries, Apply issues a fixed set of statements — never one per
// series or per event.
...
func (d *scriptedDB) Query(_ context.Context, sql string, args ...any) (pgx.Rows, error) {
	...
	case strings.Contains(sql, "path_events"):
...
func contractRuns(n int, hash byte) []Run {
	runs := make([]Run, n)
	for i := range n {
		r := run(int64(100+i), true, hash)
		r.ProbeID = uuid.MustParse(fmt.Sprintf("22222222-2222-2222-2222-2222222222%02d", i))
		runs[i] = r
	}
	return runs
}
...
func TestApplyIssuesBulkStatements(t *testing.T) {
	...
	db := &scriptedDB{}
	if _, err := Apply(context.Background(), db, uuid.NameSpaceOID, contractRuns(3, 0xaa)); err != nil {
...
func TestApplyIssuesOneBulkEventInsert(t *testing.T) {
	// Every series exists with an older, different path: N changes, and
	// still ONE event insert (a query, for RETURNING) — never one per event.
	runs := contractRuns(3, 0xbb)
	db := &scriptedDB{}
	for _, r := range runs {
		db.lockRows = append(db.lockRows, []any{
			r.ProbeID, time.Unix(50, 0).UTC(), append([]byte(nil), make([]byte, 32)...), []byte(`[]`),
		})
	}
	...
	if len(db.queries) != 3 ||
		!strings.Contains(db.queries[2], "path_events") ||
	...
	// The scripted RETURNING echoes queue order; Changes must line up.
	for i, c := range changes {
		if c.ProbeID != runs[i].ProbeID {
			t.Errorf("changes[%d].ProbeID = %s, want %s", i, c.ProbeID, runs[i].ProbeID)
		}
	}
}
