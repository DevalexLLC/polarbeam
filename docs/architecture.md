...
Relational tables: `sites` (incl. optional `latitude`/`longitude` map coordinates, set via `site set` or Settings → Sites, both-or-neither), `networks` (named connectivity planes — `name` unique and immutable, seeded `default` row that cannot be deleted; a trust statement "agents on this name can reach each other", not IPAM), `agents` (incl. `probe_address` peers should target, `last_seen_at`, and `network_id NOT NULL` — inherited from the join token at enrollment, immutable after; move a box by re-enrolling), `targets` (kind `agent`|`external`), `probe_configs` (direct rows carry `network_id`; mesh rows inherit the mesh's), `mesh_groups` (incl. `network_id NOT NULL` — a mesh binds exactly one plane) + `mesh_members`, `join_tokens` (sha256 hash, single-use, expiring, incl. `network_id`), `certificates` (serial, revoked_at), `users` (argon2id) + `sessions` + `login_events` (append-only sign-in audit, one row per successful local or SSO login, kept forever; deliberately FK-less with username/auth_source snapshots so history and exact unique-user counts survive user deletion) + `user_activity_days` (one row per person per UTC calendar day with any authenticated dashboard request, upserted by successful logins and the five-minute session touch, `last_seen_at` at that resolution; keyed per account and day, carrying the same `identity` as `login_events` for the distinct-person counts, likewise FK-less and kept forever — the source of "last active" and the monthly active-users series, which sign-ins alone cannot give because a session outlives a month boundary), `dashboard_settings` (single row: shared latency/loss warn+crit thresholds for the map) + `path_thresholds` (per-site-pair overrides of those thresholds — unordered pair keyed on site ids, per-field NULL = inherit the global value, cascades on site delete), `banner_settings` (single row: optional shared marking text + enable flag, rendered in bands at the top and bottom of every dashboard screen), `syslog_settings` (single row: the audit-log forwarding destination — transport, host/port, framing, facility, content, failure policy, PEM trust anchors and an optional write-only client key — edited from Settings → Log forwarding and applied to the running forwarder without a restart), `outage_events`, `series_state` (hysteresis counters, restart-durable), `path_events`, `traceroute_current`.
...
Columnstore (migration 0026): cold chunks are converted to TimescaleDB's compressed columnar form by policy, which drops the per-chunk B-trees (two thirds of the raw hypertable's footprint) and stores each column with a type-specific codec. Raw `probe_results` is segmented by `(agent_id, target_id, probe_id)` — the dedupe unique index's columns must be segmentby/orderby columns, and `target_id` is free (functionally dependent on `probe_id`) while letting pair reads prune batches — ordered by `time DESC`, and compressed after 8 d: that is > the agent spool's 7 d `max_age` (a replay never lands in a compressed chunk), ≥ every raw-based cagg's 8 d refresh `start_offset` (a refresh never reads a compressed source region), > the longest raw dashboard window (7 d), and < the 14 d retention, so days 8–14 — read by nothing — compress for free. The hourly caggs (latency and stage) are compressed after 10 d, past their 8 d refresh window, segmented by their group keys and ordered by `bucket DESC` (measured 4–6× and 13× on seeded fixtures; raw compresses 13–19×). `probe_results_health_30m` stays rowstore (14 d retention on 10-day materialization chunks), and so do the daily caggs: one row per series per day gives ~10 rows per segment in a 10-day chunk, and the measured 1.6×/1.8× (the uddsketch state has no binary send/recv, so it compresses as text) did not justify a policy on relations that small. A row inserted into an already compressed chunk still works (TimescaleDB decompresses the matching segment, and the unique index holds) — it is a slow path that steady state never takes. The compose file also turns Timescale telemetry off (`timescaledb.telemetry_level=off` plus the image's `TIMESCALEDB_TELEMETRY`), enables `wal_compression=lz4`, and preloads `pg_stat_statements` for operators.
...
The Settings collections `GET /config/sites`, `/config/targets`, and
`/config/probes` retain their complete response when no list parameter is
present. Query mode adds tenant-scoped literal search, endpoint-specific
filters and sorts, stable pagination, and shared `page` metadata. Session reads
of `/config/sites/{name}`, `/config/targets/{name}`, and
`/config/probes/{id}` return the same resource objects used by their collection
or mutation responses, with inaccessible rows indistinguishable from missing
...
**M6 — Packaging, rotation, air-gap hardening (1–2 wk).** Published agent container image (non-root UID 10001, `cap_net_raw` file capability, selfcheck preflight built into `run`); control-plane release bundle = `docker save` image tarballs (server, proxy, timescaledb) + production compose file + docs; cert-rotation e2e (short-lifetime test mode); `selfcheck`; CI check that builds with the network disabled (proves vendoring is complete).
...
**M10 — Site scorecards.** Month-to-date availability and performance on the map's site card: `GET /api/v1/sites/scores` tallies the UTC calendar month from `probe_results_hourly` for every visible site in one query (both directions attributed, traceroute excluded), grading each hourly bucket against the effective *warn* thresholds inside SQL — the fourth copy of the four-layer merge, held to `thresholds.Effective` + `GradeWarn` by a DB parity test that replays the shared fixture. The SPA polls it on its own 5-minute cadence keyed on the plane filter and renders two tone-classed rows under the card's live latency; the Site dashboard repeats the two figures as stat tiles beside Incident-free time (same endpoint, its own poll, context lines naming the response month) with a collapsed "How these figures are measured" disclosure defining all three. No migration: the hourly cagg's 100 d retention already covers a month. Traffic impact is deliberately absent — nothing in the probe data measures volume.
*Verify:* on the dev stack, hovering a site shows two percentages with a "Month to date · YYYY-MM" caption and a site without results shows dashes; raising a pair's `latency_warn_us` above its average lifts that pair's sites' performance on the next poll; a scoped user's `/api/v1/sites/scores` lists only its planes' sites.
